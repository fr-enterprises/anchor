...
const insert = db.prepare(
  "INSERT OR REPLACE INTO cache (key, provider, model, request, response, status, saved_at, miss_cost) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
);
const lookup = db.prepare(
  "SELECT response, status, miss_cost FROM cache WHERE key = ?",
);
...
export function tryHit(key: string): { status: number; body: Buffer; missCost: number } | null {
  const row = lookup.get(key) as { response: Buffer; status: number; miss_cost: number } | undefined;
  if (!row) return null;
  bumpHit.run(Date.now(), key);
  return { status: row.status, body: row.response, missCost: row.miss_cost ?? 0 };
}
...
export function store(opts: {
  ...
  insert.run(opts.key, opts.provider, opts.model, opts.request, opts.response, opts.status, Date.now(), opts.missCost);
}
